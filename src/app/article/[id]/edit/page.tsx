...
export default function ArticleEditPage() {
  ...
  // TODO: 認証情報からユーザーIDを取得するように修正
  // TODO: また、ユーザーIDが取得できなければホーム画面へのリダイレクト処理を追加
  const userId = "3751a2f4-17fc-ad29-19de-2ae2fadde0de";
    ...
    const fetchData = async () => {
      try {
        const [categoriesData, articleData] = await Promise.all([
          getCategories(),
          getArticleForEdit(articleId, userId),
        ]);
        ...
        setErrorMessage("記事またはカテゴリの取得に失敗しました。");
      } finally {
        setIsLoading(false);
      }
    };
    fetchData();
  }, [articleId, userId, reset]);
  ...
  // ローディング中
  if (isLoading) {
    return <p className="text-center mt-10">読み込み中です...</p>;
  }
